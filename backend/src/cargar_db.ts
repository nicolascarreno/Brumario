...
export async function cargar_jugadores() {
    ...
    for (const fila of datos) {
      ...
      const nombrePersona = fila['Nombre'];
        try {
          const nuevaPersona = new Persona({ nombre: nombrePersona, partidos: 0, goles: 0, asistencias: 0, amarillas: 0 });
...
export async function cargar_partidos() {
  // Leer el archivo (puede ser desde un buffer, archivo local o base64)
    ...
    for (const fila of datos) {
      ...
      contar_estadisticas(titulares.concat(suplentes), golesFavor, amarillas);
    ...
    for (const [nombre, { partidos, goles, asistencias, amarillas }] of Object.entries(estadisticas)) {
      try {
        await Persona.findOneAndUpdate(
          { nombre },
          { $inc: { partidos, goles, asistencias, amarillas } },
          { new: true }
        );
      } catch (err) {
...
const estadisticas: Record<string, { partidos: number; goles: number, asistencias: number, amarillas: number }> = {};
...
function contar_estadisticas(
  ...
  for (let nombreJugador of formacion) {
    if (!estadisticas[nombreJugador]) {
      estadisticas[nombreJugador] = { partidos: 0, goles: 0, asistencias: 0, amarillas: 0 };
    }
  ...
  for (const gol of golesFavor) {
    ...
    if (!estadisticas[nombreGoleador]) {
      estadisticas[nombreGoleador] = { partidos: 0, goles: 0, asistencias: 0, amarillas: 0};
    }
    estadisticas[nombreGoleador].goles += 1;

    if (!estadisticas[nombreAsistidor]) {
      estadisticas[nombreAsistidor] = { partidos: 0, goles: 0, asistencias: 0, amarillas: 0};
    }
  ...
  for (const jugadorAmarilla of amarillas) {
    if (!estadisticas[jugadorAmarilla]) {
      estadisticas[jugadorAmarilla] = { partidos: 0, goles: 0, asistencias: 0, amarillas: 0 };
    }
